...
fn is_direct_rule(rule: &str) -> bool {
    let parts = rule_parts(rule);
    matches!(parts.first(), Some(rule_type) if rule_type.eq_ignore_ascii_case("DOMAIN-SUFFIX"))
        && matches!(parts.get(2), Some(policy) if policy.eq_ignore_ascii_case("DIRECT"))
}
...
fn collect_normalized_kcp_domains(raw: Option<&Vec<String>>) -> Vec<String> {
    let mut domains = raw
        .map(Vec::as_slice)
        .unwrap_or(&[])
        .iter()
...
fn apply_kcp_proxy_routes(mut config: Mapping, kcp_proxy: Option<IKcpProxy>, proxy_host: Option<String>) -> Mapping {
    ...
    let domains = collect_normalized_kcp_domains(kcp_proxy.domains.as_ref());
    let direct_domains = collect_normalized_kcp_domains(kcp_proxy.direct_domains.as_ref());
    ...
    rules.retain(|rule| {
        rule.as_str()
            .map(|rule| !is_kcp_proxy_rule(rule) && !is_direct_rule(rule))
            .unwrap_or(true)
    });

    if kcp_proxy.catch_all.unwrap_or(false) {
        rules.insert(0, format!("MATCH,{KCP_PROXY_NAME}").into());
        for domain in direct_domains.into_iter().rev() {
            rules.insert(0, format!("DOMAIN-SUFFIX,{domain},DIRECT").into());
        }
    } else {
...
"#;
    ...
    #[test]
    fn kcp_catch_all_with_direct_whitelist_prioritizes_direct_rules() {
        let config_str = r#"
proxies:
  - name: "regular-node"
    type: ss
...
"#;
        ...
        config = apply_kcp_proxy_routes(
            ...
            Some(IKcpProxy {
                ...
                local_port: Some(1087),
                catch_all: Some(true),
                direct_domains: Some(vec![
                    "HTTPS://bypass.example.com/path".into(),
                    "*.bypass.example".into(),
                    "bypass.example".into(),
                    "".into(),
                    "bypass.example # duplicate".into(),
                ]),
                ..IKcpProxy::template()
            }),
            Some("127.0.0.1".into()),
        ...
        // DIRECT rules come before the catch-all MATCH rule so the whitelist wins.
        assert_eq!(rules[0], "DOMAIN-SUFFIX,bypass.example.com,DIRECT");
        assert_eq!(rules[1], "DOMAIN-SUFFIX,bypass.example,DIRECT");
        assert_eq!(rules[2], "MATCH,KCP Tunnel HTTP");
        // User-supplied rules remain after our injected ones.
        assert!(rules.contains(&"DOMAIN-SUFFIX,example.com,regular-node".to_string()));
    }

    #[test]
    fn kcp_rerun_replaces_direct_whitelist_without_residue() {
        let config_str = r#"
proxies:
  - name: "regular-node"
    type: ss
rules:
  - "DOMAIN-SUFFIX,stale.example,DIRECT"
"#;

        let mut config: serde_yaml_ng::Mapping =
            serde_yaml_ng::from_str(config_str).expect("Failed to parse test yaml");

        // First apply injects a single DIRECT rule.
        config = apply_kcp_proxy_routes(
            config,
            Some(IKcpProxy {
                enabled: Some(true),
                local_port: Some(1087),
                catch_all: Some(true),
                direct_domains: Some(vec!["first.example".into()]),
                ..IKcpProxy::template()
            }),
            Some("127.0.0.1".into()),
        );

        // Rerun with a smaller whitelist: the stale entry must be dropped.
        config = apply_kcp_proxy_routes(
            config,
            Some(IKcpProxy {
                enabled: Some(true),
                local_port: Some(1087),
                catch_all: Some(true),
                direct_domains: Some(vec!["second.example".into()]),
                ..IKcpProxy::template()
            }),
            Some("127.0.0.1".into()),
        );
        ...
        assert!(rules.contains(&"DOMAIN-SUFFIX,second.example,DIRECT".to_string()));
        assert!(!rules.contains(&"DOMAIN-SUFFIX,first.example,DIRECT".to_string()));
        assert_eq!(rules.iter().filter(|r| r.ends_with(",DIRECT")).count(), 1);
    }

    #[test]
    fn kcp_disable_clears_direct_whitelist_rules() {
        let config_str = r#"
proxies:
  - name: "regular-node"
    type: ss
...
"#;
        ...
        // First enable kcp with a DIRECT whitelist.
        config = apply_kcp_proxy_routes(
            config,
            Some(IKcpProxy {
                enabled: Some(true),
                local_port: Some(1087),
                catch_all: Some(true),
                direct_domains: Some(vec!["bypass.example".into()]),
                ..IKcpProxy::template()
            }),
            Some("127.0.0.1".into()),
        );
        assert!(
            config
                ...
                .map(|rules| rules
                    .iter()
                    .any(|r| r.as_str() == Some("DOMAIN-SUFFIX,bypass.example,DIRECT")))
                .unwrap_or(false)
        );

        // Disabling kcp wipes both the kcp-injected DIRECT rule and any
        // user-authored DOMAIN-SUFFIX,*,DIRECT rules, since kcp's whitelist
        // is the only DIRECT whitelist this feature is aware of.
        config = apply_kcp_proxy_routes(
            config,
            Some(IKcpProxy {
                enabled: Some(false),
        ...
        assert!(!rules.contains(&"DOMAIN-SUFFIX,bypass.example,DIRECT".to_string()));
        assert!(!rules.contains(&"DOMAIN-SUFFIX,keep.example,DIRECT".to_string()));
    }
