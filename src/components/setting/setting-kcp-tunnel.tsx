...
const normalizeConfig = (value?: IKcpProxyConfig): IKcpProxyConfig => ({
  ...
  direct_domains: value?.direct_domains ?? [],
})
...
const KcpTunnelSetting = () => {
  ...
  const [directDomainText, setDirectDomainText] = useState('')
  ...
  const openDialog = () => {
    ...
    setDirectDomainText(toDomainText(current.direct_domains))
    setTestLog('')
    setOpen(true)
    void refreshStatus()
  }
  ...
  const buildConfig = (): IKcpProxyConfig => ({
    ...
    direct_domains: fromDomainText(directDomainText),
  })
        ...
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              ...
              规则，全量经由 kcptun；下方 DIRECT 白名单中的域名仍会走直连。
              未勾选时仅 Priority domains 中的域名会优先走 kcptun。
            </Typography>
          </Box>
          ...
          {value.catch_all ? (
            <TextField
              size="small"
              label="DIRECT whitelist"
              value={directDomainText}
              onChange={(e) => setDirectDomainText(e.target.value)}
              helperText="每行一个域名；这些域名会绕过 kcptun 直接连接（DIRECT），其余接管流量仍走 kcptun。"
              multiline
              minRows={5}
              fullWidth
            />
            ...
            <TextField
              ...
              helperText="每行一个域名；这些域名会优先通过 kcptun，其余流量按原 Clash 规则处理。"
              multiline
              minRows={5}
              fullWidth
            />
